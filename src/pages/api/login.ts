  ...
  if(req.method === "POST"){
    ...
    if(usuariosEncontrados && usuariosEncontrados.length > 0){
      ...
      const token = jwt.sign({ _id :  usuarioEncontrado._id}, MINHA_CHAVE_JWT)
          return res.status(200).json({dados : {id : usuarioEncontrado._id, nome : usuarioEncontrado.nome, email : usuarioEncontrado.email, seguidores: usuarioEncontrado.seguidores, seguindo: usuarioEncontrado.seguindo, publicacoes: usuarioEncontrado.publicacoes, token, },});
    }
